...
export default function League() {
  ...
  return (
      ...
      {open && (
        <div className="absolute z-10 mt-2 w-full border rounded bg-white shadow">
          {leagues.map((option, index) => (
            ...
            >
              ...
              <span className="border-none">{option.name}</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="flex justify-start items-center space-x-2  my-4">
          ...
          <select className="p-2 border rounded-md bg-white shadow-sm focus:outline-none w-full sm:w-auto" onChange={(e)=>handleSelectedSeason(e)} value={selectedSeason}>
            {

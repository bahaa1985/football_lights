...
import {
  getLeagueTranslationByCountry,
  getRoundTranslation,
} from "../Translation/leagues.js";
          ...
          <>
            {
              ...
              type === "day_matches" ? (
                <div className="flex justify-start items-center my-2 p-2 gap-2 bg-orange-300 text-slate-600 shadow-md border border-slate-300">
                  <img
                    alt=""
                    referrerPolicy="no-referrer"
                    src={fixtures[elem][0].league.logo}
                    className="ml w-8 h-8 sm:w-10  sm:h-10"
                  />
                  <NavLink
                    className="my-auto ml-2"
                    to={`/leagues/${fixtures[elem][0].league.id}`}
                  >
                    <span className="text-left border-none font-semibold">
                      {lang === "ar"
                        ? getLeagueTranslationByCountry(
                            fixtures[elem][0]?.league.country,
              ...
              return (
                <div key={i} className="w-full mx-auto my-1 shadow-md">
                  {type === "fav_teams_matches" ? (
                    <div className="flex justify-start items-center my-2 p-2 bg-slate-400 text-slate-50 rounded-lg">
                      <img
                        ...
                        referrerPolicy="no-referrer"
                        className="ml w-8 sm:w-10 h-8 sm:h-10"
                      />
                      <NavLink
                        className="my-auto ml-2"
                        to={`/league/${elem.league.id}/${elem.league.season}`}
                      >
                      ...
                      <span className="border-none">
                        {" "}
                        {getRoundTranslation(elem.league.round, lang)}
                      </span>
                    </div>
                  ) : null}
                  ...
                  <div className="flex flex-row justify-between w-full px-2  my-1">
                      ...
                      <div className="flex flex-row justify-start items-center">
                        {
                          ...
                          elem.fixture.short === "INT" ? (
                            <div className="flex flex-row justify-start items-start">
                              <span className="text-sm font-bold border-none text-red-700 animate-pulse">
                                '{elem.fixture.status.elapsed}
                              </span>
                            </div>
                          ) : (
                    ...
                    {Date.parse(elem.fixture.date) - Date.now() <=
                    1000 * 60 * 30 ? (
                  ...
                  >
                    ...
                    <div className="w-full md:w-[48%] flex flex-row justify-center gap-2 lg:justify-between lg:gap-0 items-center my-1">
                      ...
                      >
                        <span className="border-none text-sm  lg:text-lg">
                          {lang === "ar"
                            ? getTeamByName(elem.teams.home.name)
                    ...
                    {/* Away team */}
                    <div
                      className={`w-full md:w-[48%] flex flex-row justify-center gap-2 lg:justify-between lg:gap-0 items-center my-1 ${deviceWidth > 600 ? "flex-row-reverse" : null}`}
                    >
                      <img
                      ...
                      >
                        <span className="border-none text-sm  lg:text-lg">
                          {lang === "ar"
                            ? getTeamByName(elem.teams.away.name)

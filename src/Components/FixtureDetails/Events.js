    ...
    return(
            <div key={index} className="flex space-x-3 w-64">           
                <div className="w-12">{
                    ...
                    <FontAwesomeIcon icon={faSoccerBall} size="2x" color="green"/>:
                ...
                <div className="flex flex-col justify-center">
                    ...
                    {type==="Var"?<span className="border-none">{detail}</span>: null}
                    {comments ? <span className="border-none">{comments}</span> : null}
                </div>         
            </div>)
    }
                ...
                events.map((elem,index)=>{
                    return(
                        <div className={`flex space-x-3 ${elem.team.id === teams.home.id ? "justify-start"  :"flex-row-reverse" } my-4 `} key={index}>
                            <span className="border-none">`{elem.time.elapsed}</span>
                            <div>
                            {                                        

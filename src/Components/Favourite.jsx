...
export default function Favourite(props) {
  ...
  function handlePreference() {
    ...
    if (preferedItemsArr !== null) {
      if (
        preferedItemsArr.find((item) => item.id === elem_id) === undefined &&
        preferedItemsArr.length < 5
      ) {
        preferedItemsArr.push(obj);
      } else {
        const index = preferedItemsArr.indexOf(
          preferedItemsArr.filter((item) => item.id === elem_id)[0]
        );
        preferedItemsArr = preferedItemsArr
          .slice(0, index)
          .concat(preferedItemsArr.slice(index + 1));
      }
      setCookie(cookie_name, preferedItemsArr);
    }
  }

  function setPreferedItemsColor(elem_id) {
    //to mark prefered league/team
    let preferedItemsArr = getCookie(cookie_name);
